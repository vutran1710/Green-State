...
export class Store<T, D> extends BaseStore<T> {
  ...
  get useDerivedValue(): <L extends keyof D>(k: L) => GValueReturn<D, L>[0] {
    const useDerivedValue = <L extends keyof D>(k: L) =>
      this.derivedStore.useGValue(k)[0]
    return useDerivedValue
  }

  get useDerivedState(): <L extends keyof D>(
    ...k: L[]
  ) => GStateReturn<D, L>[0] {
    const useDerivedState = <L extends keyof D>(...k: L[]) =>
      this.derivedStore.useGState(...k)[0]
